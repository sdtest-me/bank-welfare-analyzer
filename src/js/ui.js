const AppState={lang:'ru',chart:null,spiralChart:null};
...
function fmtNum(v){return v.toLocaleString(AppState.lang==='ru'?'ru-RU':'en-US');}
...
function doAnalyze(){
  ...
  showRes();
  $('hTitle').textContent=`${d.bn} | ${tr[AppState.lang].app}`;
  refresh(d);
...
function renderDetailedAnalysis(data, population, bank){
  ...
  html+=`<div><strong>🏦 ${t.bankText} (${data.bn}):</strong>
    ...
    <div class="dominant-text">${t.dominant} ${icons[bankD]} ${t.stages[bankD]} (${bank[bankD]}%) - ${bankD==='red'?(AppState.lang==='ru'?'Агрессивное извлечение прибыли':'Aggressive Profit Extraction'):(AppState.lang==='ru'?'Развитие/Инновации':'Development/Innovation')}</div></div></div>`;
...
function initSpiralChart(population,bank){
  if(AppState.spiralChart)AppState.spiralChart.destroy();
  const ctx=$('spiralChart').getContext('2d');
  AppState.spiralChart=new Chart(ctx,{type:'line',data:{labels:tr[AppState.lang].spiralLabels,datasets:[{label:tr[AppState.lang].popChartLabel,data:[population.beige,population.purple,population.red,population.blue,population.orange,population.green,population.yellow,population.turquoise],borderColor:'#16a34a',borderWidth:4,pointBackgroundColor:'#16a34a',pointRadius:6,pointHoverRadius:8,tension:0.4,fill:false},{label:tr[AppState.lang].bankChartLabel,data:[bank.beige,bank.purple,bank.red,bank.blue,bank.orange,bank.green,bank.yellow,bank.turquoise],borderColor:'#dc2626',borderWidth:4,pointBackgroundColor:'#dc2626',pointRadius:6,pointHoverRadius:8,tension:0.4,fill:false}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top',labels:{usePointStyle:true,padding:20,font:{size:12}}},tooltip:{mode:'index',intersect:false,callbacks:{label: ctx => `${ctx.dataset.label}: ${ctx.parsed.y}%`}}},scales:{y:{beginAtZero:true,max:45,ticks:{stepSize:10}},x:{grid:{display:false}}}}});
}

function refresh(d=null){
  if(!d)d=getData();
  $('rProfit').textContent=fmtPct(d.pg);$('rCap').textContent=d.cp;
  $('rDiv').textContent=d.di;$('rInt').textContent=`${d.im}–${d.ix}`;
  $('rInc').textContent=fmtPct(d.ig);$('rPov').textContent=d.pr;
  $('rGdp').textContent=fmtNum(d.gd);$('rSal').textContent=fmtNum(d.as);
  ...
  if(!AppState.chart){
    ...
    AppState.chart=new Chart(ctx,{type:'doughnut',data:{labels:[tr[AppState.lang].cons,tr[AppState.lang].bus,tr[AppState.lang].other],datasets:[{data:[d.cc,d.cb,d.co2],backgroundColor:['#dc2626','#16a34a','#64748b'],borderWidth:0,spacing:2}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{usePointStyle:true,padding:20,font:{size:11}}},tooltip:{callbacks:{label:c=>c.label+': '+c.parsed+'%'}}},cutout:'65%'}});
  }else{AppState.chart.data.labels=[tr[AppState.lang].cons,tr[AppState.lang].bus,tr[AppState.lang].other];AppState.chart.data.datasets[0].data=[d.cc,d.cb,d.co2];AppState.chart.update();}
  ...
  if(gap>5&&d.cc>40){wb.style.background='#fef2f2';wb.style.color='var(--d)';wt.textContent=AppState.lang==='ru'?`Прибыль банка растёт в ${Math.round(gap)} раз быстрее доходов населения. Высокая доля потребительских кредитов (${d.cc}%) указывает на кредитование бедности, а не развития.`:`Bank profit grows ${Math.round(gap)}x faster than population income. High share of consumption loans (${d.cc}%) indicates lending to poverty, not development.`;}
  else if(sc>=70){wb.style.background='#dcfce7';wb.style.color='#166534';wt.textContent=AppState.lang==='ru'?`✅ Банк показывает сбалансированную модель: умеренные ставки, фокус на бизнес-кредитование и рост доходов населения.`:`✅ Bank shows balanced model: moderate rates, focus on business lending, and population income growth.`;}
  else{wb.style.background='#fef3c7';wb.style.color='#92400e';wt.textContent=AppState.lang==='ru'?`⚠️ Требуется мониторинг: отдельные показатели указывают на риски долговой нагрузки.`:`⚠️ Monitor needed: some indicators suggest debt burden risks.`;}
}
